<template>
  <div class="box">
      ...
      <div class="bottom">
        ...
        <div class="bottom_r">
          <ul>
            <li :class="bottom_r_class==0?'bottom_r_class':''" @click="bottom_r_classs(0)">未审核</li>
            <li :class="bottom_r_class==1?'bottom_r_class':''" @click="bottom_r_classs(1)">今天已审核</li>
          </ul>
    ...
    handelData () {
        this.enterpriseOrderList = []
          if (this.bottom_r_class == 0){
            // this.shoptype = true
            console.log(this.enterpriseOrder.checkOrdersList.length)
            for(let x = 0; x < this.enterpriseOrder.checkOrdersList.length;x++) {
                this.enterpriseOrderList.push(this.enterpriseOrder.checkOrdersList[x])
            }
          }else{
            for(let x = 0; x < this.enterpriseOrder.checkOrdersList.length;x++) {
              this.enterpriseOrderList.push(this.enterpriseOrder.checkOrdersList[x])
    ...
    bottom_r_classs (num) {
       if (num === 1) { 
        ...
        if(this.companyname === 0) {
           this.params.supplierId = this.enterpriseName.supplierList[0].id
        }
       }else{
